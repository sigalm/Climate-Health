...
# convert to conditional probabilities IF HAVE ASTHMA:
asthma_severity_prob <- c(0.086,      # Complete control
                         0.387,       # Well control
                         0.290,       # Somewhat control
                         0.194,       # Poor control
                         0.043,       # No control at all
                         0.000,       # Dead - asthma
                         0.000)       # Dead - other cause

assign_asthma_severity <- function(asthma_status){
  if (asthma_status == 1) {
    asthma_severity <- sample(v_asthma_state_names[-1], 1, prob = asthma_severity_prob)
  } else {
    asthma_severity <- v_asthma_state_names[1]
  }
  return(asthma_severity)
}

set.seed(1111)
pop_sample <- pop_sample %>%
  mutate(asthma_status = mapply(assign_asthma_severity, asthma_status))
...
library(geosphere)
coordinates <- read.table("Data/Fire/county_coordinates.txt", sep = "\t", 
...
for (i in 1:n_i) {
  pop_sample$asthma_care[i] <- 
    sample(v_asthma_therapies, size=1, prob=m_asthma_therapy_probs[as.character(pop_sample$asthma_status[i]), ])
}
...
m_fire <- readRDS("Data/Fire/fire_data_28apr2023.rds")
