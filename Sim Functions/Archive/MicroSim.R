...
MicroSim <- function(m.x,                         # microdata 
                     initStates,                  # vector of initial health states
                     n.t,                         # number of cycles (time horizon)
                     cl,                          # cycle length
                     v.n_asthma,                  # vector of asthma health state names
                     v.therapy,                   # vector of asthma therapy names
                     m.fire,                      # fire data
                     geomap,                      # geographic data, including place names and effect size by distance
                     intervention,                # intervention coverage/reach; 0 is no intervention
                     intervention_trigger,        # the level of exposure that triggers deployment of intervention
                     popGrowth,                   # population growth data, including baseline birth rate, and annual change in birth and mortality
                     d.c,                         # discount rate for costs
                     d.e,                         # discount rate for utilities
                     seed = 1,                    # starting seed number
                     debug = FALSE                # should the model run debugging tests?
)  {
