...
Probs <- function(M_it,                      # health state occupied by individual i at cycle t
  ...
  v.p.it <- risk_modifiers[from_state, ,"transition_prob"]   # Take the corresponding row from the transition probabilities matrix
  ...
  if (from_state != "50" & from_state != "100") {  # unless the person is already in one of two absolute health states (dead) in which case no adjustment is needed and the probability of staying dead is 1
    ...
    for (k in 1:length(v.n_asthma)){                  # go through each item of the vector one by one
      to_state <- v.n_asthma[k]         # get the name of the health state
      ...
      if (debug) {
        cat("from=", from_state,  "\n",
            "to=", to_state,  "\n",
            "age=", x_i$age, class(x_i$age), "\n",
            "threshold=", riskmodifiers[from_state, to_state, "age_threshold"],class(riskmodifiers[from_state, to_state, "age_threshold"]), "\n",
            "age risk", x_i$age>=riskmodifiers[from_state, to_state, "age_threshold"], "\n")
      }
      ...
      # for all probabilities (except for the "stay put" probability), calculate an adjusted probability using the baseline probability and multiplying with an overall risk modifier
       ...
       if (debug) {
        cat("M_it=", M_it,  "\n",
            ...
            "class v.x=", class(v.x), "\n",
            "v.rr=", v.rr,"\n",
            "class v.rr=", class(v.rr), "\n")
       }
      
      if (from_state != to_state){
        v.p.it[k] = min(v.p.it[k] * modifyRisk(v.x, v.rr), 1)
      }
