...
# initProbs <- c(0.907,  # No asthma
#                0.093,   # Complete control
#                0.000, # Well control
#                0.000, # Somewhat control
#                0.000,  # Poor control
#                0.000, # No control at all
#                0.000,  # Dead - asthma
#                0.000)  # Dead - other cause
...
birthRate_bl <- 0
  # (52.4/1000) * cl   # convert annual birth rate into rate for cycle length
birthRate_change <- 0               # annual change in birth rate - 0 for up to a year
# -0.035 * cl
...
riskmodifiers <- multisheet2array(path = "Data/Asthma/Transition probabilities/transition data _ 20221206.xlsx", range=("B1:I9"), x_names = v.n_asthma, y_names = v.n_asthma)
