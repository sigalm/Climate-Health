...
7.  Place individuals in households based on their neighborhood, given a mean household size (default is 4).

8. Input (or generate) year over year fire data per neighborhood.
...
MicroSim(v.M_1, n.i, n.t, m.fire, m.smoke.exposure.cum, v.n, m.x, cl, d.c, d.e, TR.out = TRUE, TS.out = TRUE, intn=FALSE, seed = 1)
```
...
2.  Matrices to track health states (`m.M`), costs (`m.C`), and health outcomes (`m.E`) are initialized.
