...
-   `v.x`: vector containing names of individual characteristics (unused)

### Define transition probabilities

All transition probabilities depend on whether a fire happens or not. `p.fire` changes over time, and also depends on rural/urban designation by a risk ratio (`rr.fire.urban`). Fire risk increases over time (`rr.fire.over.time`).
...
6.  Assign neighborhood based on rural/urban designation. Rural neighborhoods are A and B, urban neighborhoods are C, D, and E. Probability of living in each neighborhood can be determined using the relative population densities of each neighborhood. A neighborhood is then assigned to each individual based on their rural/urban designation and the `sample()` function.\

7.  Place individuals in households based on their neighborhood, given a mena household size of 4. 
...
2.  The matrix for tracking fires (`m.fire`) in each neighborhood over time is initialized and calculated, based on current `p.fire` and urban/rural designation of the neighborhood. 

3.  Matrices to track health states (`m.M`), costs (`m.C`), and health outcomes (`m.E`) are initialized.

4.  Run the microsimulation algorithm.
...
5.  Results are returned as a list:
    ...
    h.  `m.fire`: Matrix of size `n.neighborhood` x `n.t` containing fire exposure of each individual at each cycle.
