...
p.fire <- 0.15                   # probability that a wildfire occurs 
...
rr.fire.over.time <- 1.01        # additional fire risk each consecutive year
...
for (k in 1:nrow(m.x[m.x$rural==1, ])) {
  m.x$neighborhood[m.x$rural==1][k] <-  sample(c("A","B"),size=1,prob=c(0.65,1-0.65))
}
for (k in 1:nrow(m.x[m.x$rural==0, ])) {
  m.x$neighborhood[m.x$rural==0][k] <-  sample(c("C","D","E"),size=1,prob=c(0.35,0.45,1-0.35-0.45))
}
...
for (k in levels(m.x$neighborhood)) {
  ...
  family_size <- ceiling(neighborhood_size/4)
  print(fam_count)
...
MicroSim <- function(v.M_1, n.i, n.t, p.fire, v.n, m.x, cl, d.c, d.e, TR.out = TRUE, TS.out = TRUE, intn=FALSE, seed = 1) {
  ...
  m.fire <- matrix(nrow=n.i, ncol=n.t, dimnames = list(paste("ind", 1:n.i), paste("cycle",1:n.t)))   
  ...
  for (i in 1:n.i) {
    ...
    for (t in 1:n.t) {
      p.fire <- min(p.fire*rr.fire.over.time, 1)
      fire_in_neighborhood <- p.fire
      
      m.fire[i, t] <- rbinom(1,size=1,prob=(
        m.x$rural[i]*p.fire + 
          (1-m.x$rural[i])*min(p.fire*rr.fire.urban,1)))                                    # get number of fires per cycle conditional on urban/rural designation 
      v.p <- Probs(M_it=m.M[i,t], m.fire_it=m.fire[i, t], m.chars_i=m.x[i, ])               # calculate the transition probability for individual i at cycle t
      ...
      if (m.fire[i, t]>=1) {m.x$exposure[i] <-  TRUE}        # if there has been a fire, set exposure history to TRUE, otherwise keep the same
